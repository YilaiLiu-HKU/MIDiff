...
def recover_traces( MEAN_FACTOR=1.2, output_suffix=""):
    ...
    image_list=np.load('/data/yilai/MiDiff/ckpt/ckpt/tiff_log_attnBoth/ema_0.9999_042500.pt_samples_3000x256x160x1.npz')['arr_0']
    #import pdb;pdb.set_trace()
...
# 用法示例
    ...
    output_path = f"/data/yilai/MiDiff/ckpt/ckpt/tiff_log_attnBoth/ema042500.npz"
    ...
    # 标记每一行是否出现了列 n>0 的 1
    has_poi_rows_per_image = np.any(poi_traces_arr[:, :, 1:] == 1, axis=2)  # (N, T)4
