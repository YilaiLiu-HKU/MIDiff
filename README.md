...
The `samples/` directory contains a 5-sample perturbed toy subset for checking the data shape and C-GASF conversion interface. It keeps only the two keys used by C-GASF:
...
The sample is generated by randomly selecting five traces, adding small Gaussian noise to non-zero traffic values, and shifting a subset of discrete POI labels by +/-1 within the valid label range. It is intended only as a runnable format example.
