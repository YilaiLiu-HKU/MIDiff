...
# ===== 1. (新增) 计算90%分位数 =====
print("Calculating 90th percentile of all non-zero traffic data...")
# 找到所有非零值
non_zero_values = app_traffics[app_traffics > 0]

global_quantile_90 = 0 # 初始化
if non_zero_values.size > 0:
    # 计算90%分位数
    global_quantile_90 = np.percentile(non_zero_values, 90)
    
    print(f"90th percentile (global_quantile_90) is: {global_quantile_90}")
    
    # 2. (新增) 基于分位数创建 *临时的* 裁剪后数组 (仅用于计算全局max)
    # np.clip(a, a_min=None, a_max=global_quantile_90) 会保留所有 <= a_max 的值
    clipped_app_traffics_for_max = np.clip(app_traffics, a_min=None, a_max=global_quantile_90)
    print("Temporary clipping for global max calculation completed.")

else:
    # 边缘情况：如果所有数据都是0
    print("No non-zero data found. Skipping clipping.")
    clipped_app_traffics_for_max = app_traffics
feature_maxes = np.max(app_traffics, axis=(0, 1))  # 形状 (num_features,)
...
feature_maxes=log_feature_maxes
"""# 优化1: 向量化的RGB到灰度转换
...
# 用法示例
    ...
    output_path = f"/data/yilai/MiDiff/ckpt/ckpt/tiff_log_thr/ema046000_90.npz"
